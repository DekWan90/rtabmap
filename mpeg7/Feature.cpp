...
std::shared_ptr<XM::ColorStructureDescriptor> Feature::getColorStructureD( std::shared_ptr<Frame> f, int descSize )
{
	//std::cout<< "\nColor Structure Descriptor (CSD):" <<std::endl;


	std::shared_ptr<XM::ColorStructureExtractionTool> csdt( new XM::ColorStructureExtractionTool() );
	std::shared_ptr<XM::ColorStructureDescriptor> csd( csdt->GetDescriptor() );
...
std::shared_ptr<XM::ScalableColorDescriptor> Feature::getScalableColorD( std::shared_ptr<Frame> f, bool maskFlag, int numCoeff, int bitPlanesDiscarded )
{
	...
	std::shared_ptr<XM::ScalableColorExtractionTool> scd( new  XM::ScalableColorExtractionTool( numCoeff, bitPlanesDiscarded ) );
	std::shared_ptr<XM::ScalableColorDescriptor> scdi( scd->GetDescriptor() );
	scd->extract( f->image );
...
std::shared_ptr<XM::ScalableColorDescriptor> Feature::getGoFColorD( std::vector<Mat> frames, int numCoeff, int bitPlanesDiscarded )
{
	if( frames.size() < 1 ){
	...
	std::shared_ptr<Frame> f( new Frame( frames[0].cols, frames[0].rows ) );
	f->setMaskValue(255);

	std::shared_ptr<XM::GoFGoPColorExtractionTool> gof( new XM::GoFGoPColorExtractionTool() );
	...
	std::shared_ptr<XM::ScalableColorDescriptor> scdi( gof->GetDescriptor() );
...
GoF::GoF(int type,  int w, int h, int aggregationType, int numCoeff, int bitPlanesDiscarded )
{
	...
	if( this->type == 1) {
		if( w > 0 && h > 0 ) {
			f.reset( new Frame( w, h, true, false, true ) );
...
void GoF::addFrame2( std::shared_ptr<Frame> frame )
{
	if(this->type != 2 ){
