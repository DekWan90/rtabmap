 ...
 * Update: 2009-11-08
 * Update: 2009-11-18
 * Update: 2011-02-02 (upgrade to OpenCV 2.2 -- cv::Mat)
...
2. Homogeneous Texture Descriptor, Feature::getHomogeneousTextureD, uses grayscale image as input, therefore you need to convert your color image to gray first
   Suppose you have the RGB color image 'cv::Mat img',
			// create a single channel image 'cv::Mat gray' to store the grayscale image
...
public:
	...
	static std::shared_ptr<XM::ColorStructureDescriptor> getColorStructureD( std::shared_ptr<Frame> f, int descSize = 64 );
	...
	static std::shared_ptr<XM::ScalableColorDescriptor> getScalableColorD( std::shared_ptr<Frame> f,  bool maskFlag = true, int numCoeff = 256, int bitPlanesDiscarded = 0);
	...
	static std::shared_ptr<XM::ScalableColorDescriptor> getGoFColorD( std::vector<cv::Mat> frames, int numCoeff = 256, int bitPlanesDiscarded = 0 );
...
public:
	...
	void addFrame2( std::shared_ptr<Frame> f );
